...
const Bookingserviceitem = ({ data }) => {
  ...
  const deletebooking = () => {
    axios.delete(`/deletebooking/${data?._id}`).then((res) => {
    ...
    <>
      <tr>
        ...
        <td>
          <span
            className={` ${
              data?.bookingStatus === "reject"
                ? "bg-red-500"
                : data?.bookingStatus === "accept"
                ? " bg-green-500 "
                : "bg-amber-500"
            }  text-black rounded-full font-semibold capitalize p-1 px-2 `}
          >
            {data?.bookingStatus}
          </span>
        </td>
        ...
        <th>
          <button
            ...
            className="btn hover:bg-red-600 capitalize hover:text-white btn-xs"
          >
            Delete
          </button>
        </th>
